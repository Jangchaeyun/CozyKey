...
const ApplicationCard = ({
  ...
  const [imgSrc, setImgSrc] = useState(
    application.property.photoUrl?.[0] || "/placeholder.jpg"
  );

  const statusColor =
    application.status === "Approved"
      ? "bg-green-500"
      : application.staus === "Denied"
      ? "bg-red-500"
      : "bg-yellow-500";
  ...
  return (
    <div className="border rounded-xl overflow-hidden shadow-sm bg-white mb-4">
      <div className="flex flex-col lg:flex-row items-start justify-between px-6 md:px-4 py-6 gap-6 lg:gap-4">
        ...
        <div className="flex flex-col justify-between w-full lg:basis-2/12 lg:h-48 py-2 gap-3 lg:gap-0">
          ...
          <div className="flex justify-between">
            <span className="text-gray-500">시작 일:</span>{" "}
            {new Date(application.lease?.startDate).toLocaleDateString()}
          </div>
          <div className="flex justify-between">
            <span className="text-gray-500">마지막 일:</span>{" "}
        ...
        <div className="hidden lg:block border-[0.5px] border-primary-700 h-48" />

        <div className="flex flex-col justify-start gap-5 w-full lg:basis-3/12 lg:h-48 py-2">
          <div>
            <div className="text-lg font-semibold">
              {userType === "manager" ? "세입자" : "매니저"}
            </div>
            <hr className="mt-3" />
          </div>
          <div className="flex gap-4">
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="text-sm flex item-center text-primary-600">
