...
const PropertyTenants = () => {
  ...
  const getCurrentMonthPaymentStatus = (leaseId: number) => {
    ...
    return currentMonthPayment?.paymentStatus || "지불되지 않음";
  };
  ...
  return (
    <div className="dashboard-container">
      ...
      >
        ...
        <span>부동산으로 돌아가기</span>
      </Link>
      <Header
        title={property?.name || "나의 부동산"}
        subtitle="이 부동산의 세입자와 임대 계약을 관리합니다."
      />
      ...
      <div className="w-full space-y-6">
        <div className="mt-8 bg-white rounded-xl shadow-md overflow-hidden p-6">
          <div className="flex justify-between items-center mb-4">
            <div>
              <h2 className="text-2xl font-bold mb-1">세입자 개요</h2>
              <p className="text-sm text-gray-500">
                이 부동산의 모든 세입자를 관리하고 확인하세요
              </p>
            </div>
          </div>
          <button
            className={`bg-white border border-gray-300 text-gray-700 py-2 px-4 rounded-md flex items-center justify-center hover:bg-primary-700 hover:primary-50`}
          >
      ...
      <div className="overflow-x-auto">
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead>거주자</TableHead>
              <TableHead>임대 기간</TableHead>
              <TableHead>월세</TableHead>
              <TableHead>현재 월 상태</TableHead>
              <TableHead>연락</TableHead>
          ...
          <TableBody>
            {leases?.map((lease) => (
              <TableRow key={lease.id} className="h-24">
                ...
                <TableCell>{lease.rent.toLocalString()}원</TableCell>
                <TableCell>
                  <div
                ...
                <TableCell>
                  <button
                    className={`border border-gray-3000 text-gray-700 py-2 px-4 rounded-md flex items-center justify-center font-semibold hover:bg-primary-700 hover:text-primary-50`}
                  >
